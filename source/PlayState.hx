...
class PlayState extends MusicBeatState
{
	...
	public var sky:BGSprite; //for lua position tweaking
	public var hills:BGSprite;
	public var gates:BGSprite;
	public var grass:BGSprite;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'stage': //stagef rof afsd;
				...
				*/
			case 'houseInvaded': //week 1
				sky = new BGSprite('bg/sky_fucked', -350, -180, 0.4, 0.2);
				add(sky);

				hills = new BGSprite('bg/hills', -250, 60, 0.3, 0.4);
				ezScale(hills, 1.4, 1.4);
				hills.flipX = true;
				add(hills);

				gates = new BGSprite('bg/gate', -140, 40);
				ezScale(gates, 1.4, 1.4);
				add(gates);
		...
		if(curStage == 'awesome')
		{
			...
			var awesomeGf:BGSprite = new BGSprite('best_gf', GF_X, GF_Y);
			ezScale(awesomeGf, 1.5);
			awesomeGf.antialiasing = false;
			add(awesomeGf);
		}
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				case 'invasion' | 'error' | 'choco' | 'goose' | 'equivocation' | 'long-thong':
					startDialogue(DialogueBoxPsych.parseDialogue(Paths.json(songName + '/dialogue'))); //im gonna die
				default:
					startCountdown();
			}
			seenCutscene = true;
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				var swagNote:Note = new Note(daStrumTime, daNoteData, oldNote, false, false, gottaHitNote ? SONG.player1 : SONG.player2);
				...
				if(floorSus > 0) {
					for (susNote in 0...floorSus+1)
					{
						...
						var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + (Conductor.stepCrochet / FlxMath.roundDecimal(songSpeed, 2)), daNoteData, oldNote, true, false, gottaHitNote ? SONG.player1 : SONG.player2);
	...
	function eventNoteEarlyTrigger(event:EventNote):Float {
		...
		switch(event.event) {
			...
			case 'Play Animation':
				if(event.value1 == 'smash')
					return (5/24) * 1000; //plays like 5 frames before the acutal thing so the smash lines up with the event note
		}
		return 0;
	}
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			var babyArrow:StrumNote = new StrumNote(ClientPrefs.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X, strumLine.y, i, player, (player == 1) ? SONG.player1 : SONG.player2);
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (strumScroll) //Downscroll
				{
					...
					{
						daNote.alpha = 0.5;
						daNote.distance = (0.45 * (Conductor.songPosition - daNote.strumTime) * (songSpeed * 1.5));
					}
				}
				else //Upscroll
				{
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Change Character':
				...
				if(SONG.song == 'long-thong' && daveNOT != null && bambiNOT != null)
				{
					camOther.flash(FlxColor.WHITE, 0.5);
					if(!(!bambiNOT.visible && !daveNOT.visible)) //if they arent both invisible
					{
						bambiNOT.visible = !bambiNOT.visible;
						daveNOT.visible = !daveNOT.visible;
					}
					else
						daveNOT.visible = true;
				}
	...
	public function ezScale(object:FlxSprite, scaleX:Float, scaleY:Float = 1)
	{
		//var prevPos:Array<Float> = [object.x, object.y];
		object.setGraphicSize(Std.int(object.width * scaleX));
		//object.x = prevPos[0];
		//object.y = prevPos[1];
	}
